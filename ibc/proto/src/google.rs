	...
	mod sealed {
		...
		impl borsh::BorshSerialize for Any {
			fn serialize<W: borsh::maybestd::io::Write>(
				...
				let inner_any =
					InnerAny { type_url: self.type_url.clone(), value: self.value.clone() };
		...
		impl borsh::BorshDeserialize for Any {
			fn deserialize_reader<R: Read>(reader: &mut R) -> borsh::maybestd::io::Result<Self> {
				...
				Ok(Any { type_url: inner_any.type_url, value: inner_any.value })
			}
		}
		...
		impl parity_scale_codec::Decode for Any {
			fn decode<I: parity_scale_codec::Input>(
				...
				Ok(Any { type_url: inner_any.type_url.clone(), value: inner_any.value })
			}
		}
		...
		impl scale_info::TypeInfo for Any {
			...
			fn type_info() -> scale_info::Type {
				scale_info::Type::builder()
					...
					// i128 is chosen before we represent the timestamp is nanoseconds, which is
					// represented as a i128 by Time
					.composite(
