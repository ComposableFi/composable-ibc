//! File source: solana/ledger/src/shred/traits.rs

use crate::solana::shred::{CodingShredHeader, DataShredHeader, Error, ShredCommonHeader};
use alloc::vec::Vec;
...
pub(super) trait ShredData: for<'a> Shred<'a> {
	...
	fn parent(&self) -> Result<Slot, Error> {
		...
		if parent_offset == 0 && slot != 0 {
			return Err(Error::InvalidParentOffset { slot, parent_offset });
		}
		slot.checked_sub(Slot::from(parent_offset))
			.ok_or(Error::InvalidParentOffset { slot, parent_offset })
	}
