//! File source: solana/ledger/src/shred/shred_code.rs
...
use crate::solana::{
	...
	shred::{
		...
		CodingShredHeader, ShredCommonHeader, ShredType, SignedData, DATA_SHREDS_PER_FEC_BLOCK,
		SIZE_OF_NONCE,
	},
	Error,
};
...
use solana_sdk::{clock::Slot, hash::Hash, packet::PACKET_DATA_SIZE, signature::Signature};
...
pub(super) fn sanitize<T: ShredCodeTrait>(shred: &T) -> Result<(), Error> {
	...
	if common_header.index as usize >= MAX_CODE_SHREDS_PER_SLOT {
		return Err(Error::InvalidShredIndex(ShredType::Code, common_header.index));
	}
	let num_coding_shreds = usize::from(coding_header.num_coding_shreds);
	if num_coding_shreds > 8 * DATA_SHREDS_PER_FEC_BLOCK {
		return Err(Error::InvalidNumCodingShreds(coding_header.num_coding_shreds));
	}
...
pub(super) fn erasure_mismatch<T: ShredCodeTrait>(shred: &T, other: &T) -> bool {
	let CodingShredHeader { num_data_shreds, num_coding_shreds, position: _ } =
		shred.coding_header();
