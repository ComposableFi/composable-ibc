//! File source: solana/ledger/src/shred/shred_data.rs
...
use crate::solana::{
	...
	shred::{
		...
		DataShredHeader, ShredCommonHeader, ShredFlags, ShredType, ShredVariant, SignedData,
	},
	Error,
};
use alloc::vec::Vec;
...
pub(super) fn sanitize<T: ShredDataTrait>(shred: &T) -> Result<(), Error> {
	...
	if common_header.index as usize >= MAX_DATA_SHREDS_PER_SLOT {
		return Err(Error::InvalidShredIndex(ShredType::Data, common_header.index));
	}
