...
pub struct ContextReadOnly<'a, H> {
	pub deps: Deps<'a>,
	pub env: Env,
...
impl<'a, H> PartialEq for ContextReadOnly<'a, H> {
	fn eq(&self, _other: &Self) -> bool {
		true
	}
}

impl<'a, H> Eq for ContextReadOnly<'a, H> {}

impl<'a, H> Debug for ContextReadOnly<'a, H> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		write!(f, "ContextReadOnly {{ deps: DepsMut }}")
	}
}

impl<'a, H> Clone for ContextReadOnly<'a, H> {
	fn clone(&self) -> Self {
		panic!("ContextReadOnly is not cloneable")
	}
}

impl<'a, H> ContextReadOnly<'a, H> {
	...
	H: Clone,
{
	...
	pub fn consensus_state_prefixed(
		...
		ContextReadOnly::<H>::decode_consensus_state(&bytes).map_err(|e| {
...
impl<'a, H: HostFunctionsProvider + 'static> ReaderContext for ContextReadOnly<'a, H> {}
