...
use grandpa_client_primitives::{
	justification::{GrandpaJustification, AncestryChain}, parachain_header_storage_key, FinalityProof,
	ParachainHeaderProofs, ParachainHeadersWithFinalityProof,
};
use grandpa_prover::{GrandpaProver, JustificationNotification, host_functions::HostFunctionsProvider};
...
// #[tokio::test]
// async fn test_continuous_update_of_grandpa_client2() {
// 	// env_logger::builder()
// 	// 	.filter_module("grandpa", log::LevelFilter::Trace)
// 	// 	.format_module_path(false)
// 	// 	.init();
...
// 		let header = Header {
// 			finality_proof: proof.clone().finality_proof,
// 			parachain_headers: proof.clone().parachain_headers.clone(),
// 			height: Height::new(prover.para_id as u64, finalized_para_header.number as u64),
// 		};
...
use anyhow::anyhow;
#[tokio::test]
async fn test_continuous_update_of_grandpa_client_xxx() {
	env_logger::builder()
		.filter_module("grandpa", log::LevelFilter::Trace)
		.format_module_path(false)
		.init();
	...
	let header = Header {
		finality_proof: proof.finality_proof.clone(),
		parachain_headers: proof.parachain_headers.clone(),
		height: Height::new(prover.para_id as u64, finalized_para_header.number as u64),
	};
	...
	let proof_b_tree = headers_with_finality_proof.parachain_headers;
	...
	type H = polkadot_core_primitives::Header;
	type Host = HostFunctionsProvider;
	type B = <HostFunctionsProvider as light_client_common::HostFunctions>::BlakeTwo256;
	...
	sp_trie::verify_trie_proof::<LayoutV0<B>, _, _, _>(
		&parachain_header.extrinsics_root,
		&extrinsic_proof,
		&vec![(key, Some(&extrinsic[..]))],
	)
	.map_err(|_| anyhow!("Invalid extrinsic proof")).unwrap();
