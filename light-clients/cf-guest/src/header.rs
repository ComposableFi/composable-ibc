...
use crate::{proto, ConsensusState};
...
impl<PK: PubKey> TryFrom<&proto::Header> for Header<PK> {
	...
	fn try_from(msg: &proto::Header) -> Result<Self, Self::Error> {
		...
		let signatures = msg
			...
			.iter()
			.map(|signature| {
				let index = u16::try_from(signature.index).map_err(|_| proto::BadMessage)?;
...
impl<PK: PubKey> From<Header<PK>> for ConsensusState {
	fn from(value: Header<PK>) -> Self {
		Self {
			block_hash: ibc::core::ics23_commitment::commitment::CommitmentRoot {
				bytes: value.block_hash.0.into(),
			},
			timestamp_ns: value.block_header.timestamp_ns,
		}
	}
}
