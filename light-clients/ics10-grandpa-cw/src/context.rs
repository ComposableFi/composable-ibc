...
use ibc::{
	core::{ics02_client::events::CodeId, ics26_routing::context::ReaderContext},
	Height,
};
use ics10_grandpa::{
	...
	H: Clone,
{
	...
	pub fn store_client_state_prefixed(
		...
		let encoded = Context::<H>::encode_client_state(client_state, data, self.code_id.clone())
			.map_err(|e| {
			ContractError::Grandpa(format!("error encoding client state: {e:?}"))
		})?;
