...
super::wrap!(cf_solana_upstream::ConsensusState as ConsensusState);
...
impl ConsensusState {
	pub fn new(block_hash: &CryptoHash, timestamp_ns: NonZeroU64) -> Self {
		Self(cf_solana_upstream::ConsensusState::new(block_hash, timestamp_ns))
	}
}
...
impl ibc::core::ics02_client::client_consensus::ConsensusState for ConsensusState {
	...
	fn root(&self) -> &ibc::core::ics23_commitment::commitment::CommitmentRoot {
		...
		unsafe { core::mem::transmute(&self.0.trie_root) }
	}

	fn timestamp(&self) -> ibc::timestamp::Timestamp {
		let ns = self.0.timestamp_sec.get() * 1_000_000_000;
		ibc::timestamp::Timestamp::from_nanoseconds(ns).unwrap()
	}
...
impl From<crate::Header> for ConsensusState {
...
impl From<cf_solana_upstream::Header> for ConsensusState {
	fn from(header: cf_solana_upstream::Header) -> Self {
		Self::from(&header)
	}
}
