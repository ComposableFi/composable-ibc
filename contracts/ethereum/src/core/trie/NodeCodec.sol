...
library NodeCodec {
    ...
    function decodeNodeKind(bytes memory encoded)
        external
        pure
        returns (NodeKind memory)
    {
    ...
    function decodeNibbledBranch(NodeKind memory node)
        external
        pure
        returns (NibbledBranch memory)
    {
        ...
        uint256 nibbleLen = ((node.nibbleSize +
            (NibbleSlice.NIBBLE_PER_BYTE - 1)) / NibbleSlice.NIBBLE_PER_BYTE);
        nibbledBranch.key = Slice(
    ...
    function decodeLeaf(NodeKind memory node)
        external
        pure
        returns (Leaf memory)
    {
        ...
        leaf.key = Slice(nibbleBytes, node.nibbleSize % NIBBLE_PER_BYTE);
    ...
    function decodeExtension(NodeKind memory)
        external
        pure
    ...
    function decodeBranch(NodeKind memory)
        external
        pure
