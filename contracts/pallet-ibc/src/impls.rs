    ...
    T::Balance: From<u128>,
{
    ...
    fn channel_client_id(channel_end: &ChannelEnd) -> Result<ClientId, Error<T>> {
        ...
        let connection_end = ctx
            ...
            .map_err(|_| Error::<T>::ConnectionNotFound)?;
    ...
    T::Balance: From<u128>,
{
    fn latest_height_and_timestamp(
        ...
        let source_channel_end =
            ctx.channel_end(&(port_id.clone(), *channel_id))
    ...
    fn send_packet(data: SendPacketData) -> Result<(), IbcHandlerError> {
        ...
        let sequence = ctx
            ...
            .map_err(|_| IbcHandlerError::SendPacketError {
                msg: Some(format!(
                    "Failed to get next_sequence_send for {}",
                    source_channel
                )),
            })?;
        ...
        let send_packet_result = ibc::core::ics04_channel::handler::send_packet::send_packet(
            &ctx, packet,
        )
        .map_err(|e| IbcHandlerError::SendPacketError {
    ...
    fn open_channel(
        ...
        let channel_counter =
            ctx.channel_counter()
                .map_err(|e| IbcHandlerError::ChannelInitError {
                    msg: Some(e.to_string()),
                })?;
        ...
        let res = ibc::core::ics26_routing::handler::deliver::<_>(&mut ctx, msg).map_err(|e| {
            IbcHandlerError::ChannelInitError {
                msg: Some(e.to_string()),
            }
        })?;
    ...
    fn write_acknowledgement(packet: &Packet, ack: Vec<u8>) -> Result<(), IbcHandlerError> {
        ...
        .map_err(|e| IbcHandlerError::WriteAcknowledgementError {
            msg: Some(e.to_string()),
        })?;
        ...
        Self::deposit_event(Event::<T>::Events {
            events: vec![event],
        });
        Ok(())
    }
    ...
    fn send_transfer(
        ...
        let mut ctx = Context::<T>::default();
        send_transfer::<_, _>(&mut ctx, &mut handler_output, msg).map_err(|e| {
            IbcHandlerError::SendTransferError {
                msg: Some(e.to_string()),
            }
        })?;
