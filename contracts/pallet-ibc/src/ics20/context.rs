	...
	u32: From<<T as frame_system::Config>::BlockNumber>,
{
    ...
    fn mint_coins(
        ...
        let asset_id = T::IdentifyAssetId::to_asset_id(&denom);

        let metadata = <T::MultiCurrency as InspectMetadata<T::AccountId>>::name(&asset_id);
        ...
        if metadata.is_empty() {
            ...
            <T::MultiCurrency as Create<T::AccountId>>::create(
                asset_id,
                asset_admin,
                true,
                T::Balance::zero(),
