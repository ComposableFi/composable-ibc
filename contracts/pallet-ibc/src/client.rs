    ...
    <T as frame_system::Config>::BlockNumber: From<u32>,
{
    ...
    fn host_consensus_state(
        ...
        return Ok(AnyConsensusState::Beefy(
            ics11_beefy::consensus_state::ConsensusState {
                timestamp,
                root: vec![].into(),
            },
        ));
    ...
    u32: From<<T as frame_system::Config>::BlockNumber>,
{
    ...
    fn validate_self_client(&self, client_state: &AnyClientState) -> Result<(), ICS02Error> {
        let (relay_chain, para_id, latest_para_height) = match client_state {
            ...
            client => Err(ICS02Error::unknown_client_type(format!(
                "{}",
                client.client_type()
            )))?,
        };
