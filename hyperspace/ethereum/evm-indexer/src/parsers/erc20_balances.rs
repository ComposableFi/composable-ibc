...
impl ERC20Balances {
	...
	pub async fn parse(&self, db: &Database, transfers: &Vec<DatabaseErc20Transfer>) -> Result<()> {
		...
		let senders: Vec<(String, String, String)> = transfers
			...
			.map(|transfer| {
				(transfer.from_address.clone(), transfer.token.clone(), transfer.chain.clone())
			})
			.collect();
		...
		let receivers: Vec<(String, String, String)> = transfers
			...
			.map(|transfer| {
				(transfer.to_address.clone(), transfer.token.clone(), transfer.chain.clone())
			})
			.collect();
		...
		info!("ERC20Balances: fetched {} balances to update", stored_balances.len());

		let mut balances: HashMap<(String, String, String), DatabaseErc20Balance> = HashMap::new();

		for balance in stored_balances {
			balances.insert(
				(balance.address.clone(), balance.token.clone(), balance.chain.clone()),
				balance,
			);
		}
		...
		for transfer in transfers {
			...
			if sender != format_address(H160::zero()) {
				let id = (sender.clone(), transfer.token.clone(), transfer.chain.clone());
			...
			if receiver != format_address(H160::zero()) {
				let id = (receiver.clone(), transfer.token.clone(), transfer.chain.clone());
		...
		let mut query_builder =
			QueryBuilder::new("INSERT INTO erc20_balances (address, balance, chain, token) ");
		...
		if parsed_transfers.len() > 0 {
			...
			for (start, end) in chunks {
				let mut query_builder =
                QueryBuilder::new("INSERT INTO erc20_transfers(chain, from_address, hash, log_index, to_address, token, value) ");
		...
		if missing_tokens.len() > 0 {
			...
			info!("ERC20Balances: Fetching data for {} missing tokens data", missing_tokens.len());

			let missing_tokens_vector =
				missing_tokens.into_iter().collect::<Vec<(String, String)>>();
			...
			for chunk in chunks {
				...
				let mut query_builder = QueryBuilder::new(
					"INSERT INTO erc20_tokens (address, chain, decimals, name, symbol) ",
				);
				...
				info!("ERC20Balances: Inserted {} missing tokens data", tokens_amount);
			}
		}
		Ok(())
	}
	...
	pub async fn get_current_balances(
		...
		if balances.len() > 0 {
			let rows =
				sqlx::query_as::<_, DatabaseErc20Balance>(&query).fetch_all(connection).await;
	...
	pub async fn get_tokens(
		...
		if tokens.len() > 0 {
			let rows = sqlx::query_as::<_, DatabaseErc20Token>(&query).fetch_all(connection).await;
