...
impl ERC20Tokens {
	...
	pub async fn parse_extenal(&self, db: &Database) -> Result<()> {
		...
		for (name, chain) in chains {
			...
			for url in chain.tokens_lists {
				...
				info!("ERC20Tokens: inserted {} tokens for chain {}", tokens_amount, name);
			}
		}
		Ok(())
	}
	...
	pub async fn parse(&self, db: &Database, transfers: &Vec<DatabaseErc20Transfer>) -> Result<()> {
		...
		let mut query_builder =
			QueryBuilder::new("INSERT INTO erc20_tokens (address, chain, decimals, name, symbol) ");
		...
		for token in db_tokens {
			...
			tokens_data.push((token.address, token.chain, token.decimals.unwrap(), name, symbol));
		}
		...
		info!("ERC20Tokens: Inserted {} erc20 tokens to the database.", tokens_amount);
	...
	pub async fn get_token_metadata(
		...
		return Some(DatabaseErc20Token { address, chain, name, decimals, symbol })
	}
}
