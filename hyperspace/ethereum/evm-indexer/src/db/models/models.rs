...
impl DatabaseBlock {
	pub fn from_rpc(block: &Block<Transaction>, chain: &'static str) -> Self {
		...
		let uncles =
			block.uncles.clone().into_iter().map(|uncle| Some(format_hash(uncle))).collect();
...
pub fn byte4_from_input(input: &String) -> [u8; 4] {
	...
	let byte4: [u8; 4] = [input_bytes[0], input_bytes[1], input_bytes[2], input_bytes[3]];
...
impl DatabaseLog {
	pub fn from_rpc(log: Log, chain: String) -> Self {
		...
		Self {
			...
			chain,
			topics: log.topics.clone().into_iter().map(|topic| Some(format_hash(topic))).collect(),
...
pub struct DatabaseIBCEventData {
	pub block_number: i64,
	pub event_data: sqlx::types::Json<serde_json::value::Value>,
	pub address: Vec<u8>,
	pub topic0: Vec<u8>,
	...
	pub tx_index: i64,
	pub event_index: i64,
	pub raw_log: sqlx::types::Json<Log>,
}
