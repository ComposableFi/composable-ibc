...
impl Database {
	...
	pub async fn get_indexed_blocks(&self) -> Result<HashSet<i64>> {
		...
		let keys: Vec<String> =
			connection.keys(format!("{}*", self.chain.name.to_string())).unwrap();
	...
	async fn store_transactions_receipts(&self, receipts: &Vec<DatabaseReceipt>) -> Result<()> {
		...
		for (start, end) in chunks {
			...
			query.execute(connection).await.expect("Unable to store receipts into database");
		}
	...
	pub async fn store_contracts_information(
		...
		let chunks =
			get_chunks(contracts_information.len(), DatabaseContractInformation::field_count());
	...
	pub async fn store_methods(&self, methods: &Vec<DatabaseMethod>) -> Result<()> {
		...
		for (start, end) in chunks {
			...
			query.execute(connection).await.expect("Unable to store methods into database");
		}
...
pub fn get_chunks(num_items_to_insert: usize, column_count: usize) -> Vec<(usize, usize)> {
	...
	while chunk.1 != num_items_to_insert {
		chunk = (chunk.0 + max_item_size, min(num_items_to_insert, chunk.1 + max_item_size));
		chunks.push(chunk);
	}
	chunks
}
