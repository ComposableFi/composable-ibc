...
use sp_trie::{generate_trie_proof, LayoutV0, MemoryDB, TrieMut, TrieDBMutBuilder};
use std::{
	...
	B::Error: From<A::Error>,
{
	...
	let extrinsics_root = {
		...
		let mut trie = <TrieDBMutBuilder<LayoutV0<BlakeTwo256>>>::new(&mut para_db, &mut root).build();
	...
	let state_proof =
		prove_read_on_trie_backend(&TrieBackendBuilder::new(para_db.clone(), root).build(), &[key.as_ref()])
			.unwrap()
			.into_nodes()
			.into_iter()
			.collect::<Vec<_>>();
