...
use ibc_proto_new::{ibc::lightclients::tendermint::v1::ConsensusState, google::protobuf::Timestamp};
    ...
    solana_ibc::consensus_state::AnyConsensusState::Tendermint(cs) => {
      ...
      AnyConsensusState::Tendermint(ics07_tendermint::consensus_state::ConsensusState {
        ...
        next_validators_hash: Hash::try_from(
          cs.next_validators_hash().as_bytes().to_vec(),
        )
        .unwrap(),
      })
    },
    solana_ibc::consensus_state::AnyConsensusState::Mock(_) =>
      panic!("Mocks are not supported"),
  } 
}

pub fn convert_old_consensus_state_to_new(consensus_state: AnyConsensusState) -> solana_ibc::consensus_state::AnyConsensusState {
  match consensus_state {
    AnyConsensusState::Tendermint(cs) => {
      ...
      ConsensusState {
        timestamp: Some(Timestamp { seconds: timestamp_in_secs, nanos: remaining_timestamp_in_nano  }),
        root: Some(ibc_proto_new::ibc::core::commitment::v1::MerkleRoot { hash: cs.root.bytes }),
