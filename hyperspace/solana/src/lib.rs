...
impl Chain for SolanaClient {
	...
	async fn submit(&self, messages: Vec<Any>) -> Result<Self::TransactionId, Error> {
		...
		for message in messages {
			...
			let mantis_token_supply_on_l1 =
				self.l2_rpc.get_token_supply(&self.mantis_token_mint_pubkey).await.unwrap();
			let ix = spl_token::instruction::update_l1_token_supply(
				&self.mantis_token_program_id,
				&self.mantis_token_mint_pubkey,
				Some(&authority.pubkey()),
			)?;
			let update_mantis_supply_ix: Transaction = Transaction::new_with_payer(
				&[compute_budget_ix.clone(), ix],
				Some(&authority.pubkey()),
			);
			...
			current_transactions.push(update_mantis_supply_ix);
